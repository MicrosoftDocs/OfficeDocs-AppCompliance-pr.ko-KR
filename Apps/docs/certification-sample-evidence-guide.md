...
<a id="control-1"></a>[제어 #1](#control-1): 바이러스 백신 사례 및 절차를 제어하는 정책 설명서를 제공합니다.
...
<a id="control-2"></a>[제어 #2](#control-2): 바이러스 백신 소프트웨어가 샘플링된 모든 시스템 구성 요소에서 실행되고 있다는 입증 가능한 증거를 제공합니다.
...
<a id="control-3"></a>[제어 #3](#control-3): 바이러스 백신 서명이 모든 환경에서 최신 상태라는 입증 가능한 증거를 제공합니다(1일 이내).
...
<a id="control-4"></a>[제어 #4](#control-4): 바이러스 백신이 모든 샘플링된 시스템 구성 요소에서 액세스 시 검사 또는 정기적인 검사를 수행하도록 구성되었다는 입증 가능한 증거를 제공합니다.
...
<a id="control-5"></a>[제어 #5](#control-5): 바이러스 백신이 샘플링된 모든 시스템 구성 요소에서 맬웨어 또는 격리 및 경고를 자동으로 차단하도록 구성되었다는 입증 가능한 증거를 제공합니다.
...
<a id="control-6"></a>[제어 #6](#control-6): 애플리케이션이 배포되기 전에 승인되었다는 입증 가능한 증거를 제공합니다.
...
- 증거 지침 예제: 승인 프로세스를 따르고 있음을 보여 주는 증거를 제공할 수 있습니다. 서명된 문서, 변경 제어 시스템 내에서 추적 또는 Azure DevOps 또는 JIRA 사용하여 이러한 요청 및 권한 부여를 추적하여 제공할 수 있습니다.
...
<a id="control-7"></a>[제어 #7](#control-7): 비즈니스 정당성이 있는 승인된 애플리케이션의 전체 목록이 존재하고 유지 관리된다는 입증 가능한 증거를 제공합니다.
...
<a id="control-8"></a>[컨트롤 #8](#control-8): 애플리케이션 제어 소프트웨어가 특정 애플리케이션 제어 메커니즘을 충족하도록 구성되어 있음을 자세히 설명하는 지원 설명서를 제공합니다.
...
<a id="control-9"></a>[컨트롤 #9](#control-9): 애플리케이션 제어가 샘플링된 모든 시스템 구성 요소에서 문서화된 대로 구성되었다는 입증 가능한 증거를 제공합니다.
...
<a id="control-10"></a>[제어 #10](#control-10): 새 보안 취약성을 식별하고 위험 점수를 할당하는 방법을 제어하는 정책 설명서를 제공합니다.
...
<a id="control-11"></a>[제어 #11](#control-11): 새 보안 취약성이 식별되는 방법에 대한 증거를 제공합니다.
...
- 증거 지침 예제: 메일링 목록에 대한 구독을 표시하고, 새로 릴리스된 취약성에 대한 보안 원본을 수동으로 검토하는 방법으로 증거를 제공할 수 있습니다(예: JIRA 또는 Azure DevOps 사용하여 활동의 타임스탬프로 적절하게 추적해야 하는 경우), 오래된 소프트웨어를 찾는 도구(예: 오래된 소프트웨어 라이브러리를 찾을 때 Snyk일 수 있음) 또는 Nessus를 사용할 수 있음  오래된 소프트웨어를 식별하는 인증된 검사입니다.).
...
<a id="control-12"></a>[제어 #12](#control-12): 모든 취약성이 식별되면 위험 순위가 할당됨을 보여 주는 증거를 제공합니다.
...
<a id="control-13"></a>[제어 #13](#control-13): 위험, 높음 및 중간 위험 취약성에 적합한 최소 패치 기간을 포함하는 범위 내 시스템 구성 요소의 패치에 대한 정책 설명서를 제공합니다. 지원되지 않는 운영 체제 및 소프트웨어의 서비스 해제
...
<a id="control-14"></a>[제어 #14](#control-14): 샘플링된 모든 시스템 구성 요소가 패치되고 있다는 입증 가능한 증거를 제공합니다.
...
<a id="control-15"></a>[제어 #15](#control-15): 지원되지 않는 운영 체제 및 소프트웨어 구성 요소가 환경 내에서 사용되지 않는다는 입증 가능한 증거를 제공합니다.
...
<a id="control-16"></a>[제어 #16](#control-16): 분기별 인프라 및 웹 애플리케이션 취약성 검사 보고서를 제공합니다. 전체 공용 공간(IP 주소 및 URL) 및 내부 IP 범위에 대해 검사를 수행해야 합니다.
...
<a id="control-17"></a>[제어 #17](#control-17): 취약성 검사 중에 식별된 취약성의 수정이 문서화된 패치 기간에 따라 패치된다는 입증 가능한 증거를 제공합니다.
...
<a id="control-18"></a>[제어 #18](#control-18): 방화벽 관리 사례 및 절차를 제어하는 정책 설명서를 제공합니다.
...
<a id="control-19"></a>[컨트롤 #19](#control-19): 프로덕션 환경에 설치하기 전에 모든 기본 관리 자격 증명이 변경되었다는 입증 가능한 증거를 제공합니다.
...
<a id="control-20"></a>[제어 20](#control-20): 방화벽이 범위 내 환경의 경계에 설치되고 경계 네트워크(DMZ, 완만 영역 및 스크린된 서브넷이라고도 함) 및 내부 신뢰할 수 있는 네트워크 사이에 설치된다는 입증 가능한 증거를 제공합니다.
...
<a id="control-21"></a>[제어 21](#control-21): 모든 공용 액세스가 비무장지대(DMZ)에서 종료된다는 입증 가능한 증거를 제공합니다.
...
<a id="control-21"></a>[제어 22](#control-22): 방화벽을 통해 허용된 모든 트래픽이 승인 프로세스를 통과한다는 입증 가능한 증거를 제공합니다.
...
- 증거 지침 예: 증거는 승인되는 방화벽 변경 요청을 보여 주는 설명서 형식일 수 있으며, CAB(변경 어드바이저 위원회) 회의 또는 모든 변경 내용을 추적하는 변경 제어 시스템에서 몇 분 정도 소요될 수 있습니다.
...
<a id="control-23"></a>[컨트롤 23](#control-23): 방화벽 규칙 기반이 명시적으로 정의되지 않은 트래픽을 삭제하도록 구성되었다는 입증 가능한 증거를 제공합니다.
...
<a id="control-24"></a>[컨트롤 24](#control-24): 방화벽이 모든 비 콘솔 관리 인터페이스에서 강력한 암호화만 지원한다는 입증 가능한 증거를 제공합니다.
...
<a id="control-25"></a>[컨트롤 25](#control-25): 최소 6개월마다 방화벽 규칙 검토를 수행하고 있다는 입증 가능한 증거를 제공합니다.
...
<a id="control-26"></a>[제어 26](#control-26): WAF(Web Application Firewall)가 악의적인 트래픽을 적극적으로 모니터링, 경고 및 차단하도록 구성되었다는 입증 가능한 증거를 제공합니다.
...
<a id="control-27"></a>[컨트롤 27](#control-27): WAF가 SSL 오프로드를 지원한다는 입증 가능한 증거를 제공합니다.
...
<a id="control-28"></a>[컨트롤 28](#control-28): 'WAF가 OWASP Core 규칙 집합(3.0 또는 3.1)에 따라 다음 취약점의 일부 또는 전부로부터 보호된다는 입증 가능한 증거를 제공합니다.
- 프로토콜 및 인코딩 문제,
- 헤더 삽입, 요청 밀수 및 응답 분할,
- 파일 및 경로 순회 공격,
- RFI(원격 파일 포함) 공격,
...
<a id="control-29"></a>[컨트롤 29](#control-29): 변경 제어 프로세스를 제어하는 정책 설명서를 제공합니다.
...
<a id="control-30"></a>[제어 30](#control-30): 개발 및 테스트 환경이 프로덕션 환경과 의무를 분리하는 것을 입증할 수 있는 증거를 제공합니다.
...
<a id="control-31"></a>[제어 31](#control-31): 개발 또는 테스트 환경 내에서 중요한 프로덕션 데이터가 사용되지 않는다는 입증 가능한 증거를 제공합니다.
...
<a id="control-32"></a>[제어 32](#control-32): 문서화된 변경 요청에 변경의 영향, 백아웃 절차의 세부 정보 및 수행할 테스트가 포함되어 있음을 입증할 수 있는 증거를 제공합니다.
...
<a id="control-33"></a>[제어 33](#control-33): 변경 요청이 권한 부여 및 사인오프 프로세스를 거치는 입증 가능한 증거를 제공합니다.
...
<a id="control-34"></a>[제어 34](#control-34): OWASP 상위 10개 또는 SANS 상위 25개 CWE와 같은 일반적인 취약성 클래스에 대한 보안 코딩 모범 사례 지침을 포함하여 보안 소프트웨어 개발 및 배포를 지원하는 정책 및 절차를 제공합니다.
...
<a id="control-35"></a>[컨트롤 35](#control-35): 코드 변경 내용이 두 번째 검토자가 검토 및 권한 부여 프로세스를 진행한다는 입증 가능한 증거를 제공합니다.
...
<a id="control-36"></a>[제어 36](#control-36): 개발자가 매년 보안 소프트웨어 개발 교육을 받는다는 입증 가능한 증거를 제공합니다.
...
<a id="control-37"></a>[컨트롤 37](#control-37): 코드 리포지토리가 MFA(다단계 인증)로 보호된다는 입증 가능한 증거를 제공합니다.
...
<a id="control-38"></a>[컨트롤 38](#control-38): 코드 리포지토리를 보호하기 위해 액세스 제어가 마련되어 있다는 입증 가능한 증거를 제공합니다.
...
<a id="control-39"></a>[컨트롤 39](#control-39): 계정 관리 사례 및 절차를 제어하는 정책 설명서를 제공합니다.
...
<a id="control-40"></a>[컨트롤 40](#control-40): 기본 자격 증명이 샘플링된 시스템 구성 요소에서 비활성화, 제거 또는 변경되었다는 입증 가능한 증거를 제공합니다.
...
<a id="control-41"></a>[제어 41](#control-41): 계정 생성, 수정 및 삭제가 설정된 승인 프로세스를 거치는 입증 가능한 증거를 제공합니다.
...
<a id="control-42"></a>[컨트롤 42](#control-42): 3개월 이내에 사용되지 않는 계정을 사용하지 않도록 설정하거나 삭제하는 프로세스가 마련되어 있다는 입증 가능한 증거를 제공합니다.
...
- 증거 예제: 이 첫 번째 스크린샷은 AAD 내에서 사용자의 마지막 로그온 특성을 보기 위해 분기별로 실행되는 스크립트의 출력을 보여줍니다.

![스크린샷은 AAD 내에서 사용자의 마지막 로그온 특성을 보기 위해 분기별로 실행되는 스크립트의 출력을 보여줍니다.](../media/UserGuidePhotos/Control-No-42.png)
...
<a id="control-43"></a>[컨트롤 43](#control-43): 강력한 암호 정책 또는 사용자 자격 증명을 보호하기 위한 기타 적절한 완화 방법이 마련되었다는 입증 가능한 증거를 제공합니다.  다음은 최소 지침으로 사용해야 합니다.
- 최소 암호 길이 8자
- 10회 이하의 계정 잠금 임계값
- 최소 5개 암호의 암호 기록
- 강력한 암호 사용 적용
...
<a id="control-44"></a>[컨트롤 44](#control-44): 고유한 사용자 계정이 모든 사용자에게 발급되었다는 입증 가능한 증거를 제공합니다.
...
<a id="control-45"></a>[제어 45](#control-45): 환경 내에서 최소 권한 원칙을 따르고 있다는 입증 가능한 증거를 제공합니다.
...
<a id="control-46"></a>[제어 46](#control-46): 서비스 계정을 보호하거나 강화하기 위한 프로세스가 마련되어 있고 프로세스가 수행되고 있다는 입증 가능한 증거를 제공합니다.
...
<a id="control-47"></a>[컨트롤 47](#control-47): MFA가 모든 원격 액세스 연결 및 모든 비 콘솔 관리 인터페이스에 대해 구성되었다는 입증 가능한 증거를 제공합니다.
...
<a id="control-48"></a>[제어 48](#control-48): 모든 코드 리포지토리 및 클라우드 관리 인터페이스에 대한 액세스를 포함하여 모든 원격 액세스 연결 및 모든 비 콘솔 관리 인터페이스에 대해 강력한 암호화가 구성되었다는 입증 가능한 증거를 제공합니다.
...
<a id="control-49"></a>[컨트롤 49](#control-49): MFA가 모든 DNS(공용 도메인 이름 서비스) 레코드를 관리하고 유지 관리하는 데 사용하는 관리 포털을 보호하는 데 사용된다는 입증 가능한 증거를 제공합니다.
...
<a id="control-50"></a>[제어 50](#control-50): 범위 내 환경 경계에 IDPS(침입 감지 및 방지 시스템)가 배포되었다는 입증 가능한 증거를 제공합니다.
...
<a id="control-51"></a>[컨트롤 51](#control-51): IDPS 서명이 최신 상태로 유지된다는 입증 가능한 증거를 제공합니다(24시간 이내).
...
<a id="control-52"></a>[컨트롤 52](#control-52): 들어오는 모든 웹 트래픽에 대한 TLS 검사를 지원하도록 IDPS가 구성되었다는 입증 가능한 증거를 제공합니다.
...
<a id="control-53"></a>[컨트롤 53](#control-53): IDPS가 모든 인바운드 트래픽 흐름을 모니터링하도록 구성되었다는 입증 가능한 증거를 제공합니다.
...
<a id="control-54"></a>[제어 54](#control-54): IDPS가 모든 아웃바운드 트래픽 흐름을 모니터링하도록 구성되었다는 입증 가능한 증거를 제공합니다.
...
<a id="control-55"></a>[제어 55](#control-55): 보안 이벤트 로깅을 제어하는 모범 사례 및 절차에 대한 정책 설명서를 제공합니다.
...
<a id="control-56"></a>[컨트롤 56](#control-56): 다음 이벤트를 기록하기 위해 샘플링된 모든 시스템 구성 요소에서 보안 이벤트 로깅이 설정되었음을 보여 주는 입증 가능한 증거를 제공합니다.
...
<a id="control-57"></a>[컨트롤 57](#control-57): 기록된 보안 이벤트에 다음과 같은 최소 정보가 포함되어 있음을 입증할 수 있는 증거를 제공합니다.
- 사용자
- 이벤트 유형
- 날짜 및 시간
- 성공 또는 실패 지표
...
<a id="control-58"></a>[컨트롤 58](#control-58): 샘플링된 모든 시스템 구성 요소가 동일한 주 및 보조 서버에 시간 동기화된다는 입증 가능한 증거를 제공합니다.
...
<a id="control-59"></a>[제어 59](#control-59): 공용 시스템이 사용 중일 때 보안 이벤트 로그가 경계 네트워크 내가 아닌 중앙 집중식 로깅 솔루션으로 전송된다는 증거를 제공합니다.
...
<a id="control-60"></a>[제어 60](#control-60): 중앙 집중식 로깅 솔루션이 로깅 데이터의 무단 변조로부터 보호됨을 보여 주는 입증 가능한 증거를 제공합니다.
...
<a id="control-61"></a>[제어 61](#control-61): 최소 30일의 보안 이벤트 로깅 데이터를 즉시 사용할 수 있으며 90일의 보안 이벤트 로그가 유지된다는 입증 가능한 증거를 제공합니다.
...
<a id="control-62"></a>[제어 62](#control-62): 로그 검토 사례 및 절차를 제어하는 정책 설명서를 제공합니다.
...
<a id="control-63"></a>[제어 63](#control-63): 잠재적인 보안 이벤트를 식별하기 위해 사용자 또는 자동화된 도구에서 로그를 매일 검토한다는 입증 가능한 증거를 제공합니다.

- 의도: 이 컨트롤의 의도는 매일 로그 검토가 수행되도록 하는 것입니다. 보안 이벤트 경고를 제공하도록 구성된 경고 스크립트/쿼리에서 선택되지 않을 수 있는 변칙을 식별하는 것이 중요합니다.

- 증거 지침 예제: 일반적으로 로그 검토가 수행되고 있음을 보여 주는 스크린샷 또는 화면 공유를 통해 증명 정보를 제공합니다. 이는 매일 완료되는 양식을 통해 또는 매일 수행되는 것을 보여주기 위해 관련 댓글이 게시되는 JIRA 또는 DevOps 티켓을 통해서일 수 있습니다. 예를 들어, 매주 JIRA 티켓은 2021&quot;년 6월 26일 매일 매일 로그 검토 결과를 게시하는 일별 로그 검토 W/C를 만들 &quot;수 있습니다. 플래그가 지정된 변칙이 있으면 이 동일한 티켓 내에 문서화하여 단일 JIRA 다음 컨트롤을 모두 보여 줄 수 있습니다.
...
<a id="control-64"></a>[제어 64](#control-64): 잠재적인 보안 이벤트 및 변칙이 조사 및 수정된다는 입증 가능한 증거를 제공합니다.
...
- 증거 지침 예제: 매일 로그 검토의 일부로 식별된 변칙이 후속 조치됨을 보여 주는 스크린샷을 통해 증거를 제공해야 합니다. 위에서 이미 설명한 대로 이 작업은 플래그가 지정된 변칙을 보여 주는 JIRA 티켓을 통해 수행된 활동을 자세히 설명하는 것일 수 있습니다. 이렇게 하면 수행되는 모든 활동을 추적하기 위해 특정 JIRA 티켓이 발생하거나 일일 로그 검토 티켓 내에 문서화될 수 있습니다. 인시던트 대응 조치가 필요한 경우 인시던트 대응 프로세스의 일부로 문서화해야 하며 이를 입증하기 위해 증거를 제공해야 합니다.
...
<a id="control-65"></a>[제어 65](#control-65): 보안 이벤트 경고 사례 및 절차를 제어하는 정책 설명서를 제공합니다.
...
<a id="control-66"></a>[제어 66](#control-66): 다음 유형의 보안 이벤트에 대한 즉각적인 심사를 위해 경고가 트리거된다는 입증 가능한 증거를 제공합니다.
- 권한 있는 계정 만들기 또는 수정
- 바이러스 또는 맬웨어 이벤트
- 이벤트 로그 변조
...
- 증거 지침 예제: 경고 구성의 스크린샷 **과** 수신되는 경고의 증거를 통해 증거를 제공해야 합니다. 구성 스크린샷은 경고를 트리거하는 논리와 경고를 보내는 방법을 보여 줍니다. 경고는 SMS, 이메일, Teams 채널, Slack 채널 등을 통해 보낼 수 있습니다.
...
<a id="control-67"></a>[제어 67](#control-67): 직원이 보안 경고에 응답하기 위해 항상 하루 종일, 매일 사용할 수 있음을 보여주는 입증 가능한 증거를 제공합니다.
...
<a id="control-68"></a>[제어 68](#control-68): 공식적인 정보 보안 위험 관리 프로세스가 설정되었다는 입증 가능한 증거를 제공합니다.
...
<a id="control-69"></a>[제어 69](#control-69): 공식적인 위험 평가가 최소한 매년 발생한다는 입증 가능한 증거를 제공합니다.
...
<a id="control-70"></a>[제어 70](#control-70): 정보 보안 위험 평가에 위협, 취약성 또는 이와 동등한 요소가 포함되어 있다는 입증 가능한 증거를 제공합니다.
...
<a id="control-71"></a>[제어 71](#control-71): 정보 보안 위험 평가에 영향, 가능성 위험 매트릭스 또는 해당 항목이 포함된다는 입증 가능한 증거를 제공합니다.
...
<a id="control-72"></a>[제어 72](#control-72): 정보 보안 위험 평가에 위험 등록 및 치료 계획이 포함되어 있다는 입증 가능한 증거를 제공합니다.
...
<a id="control-73"></a>[제어 73](#control-73): IRP(보안 인시던트 대응 계획)를 제공합니다.
...
<a id="control-74"></a>[제어 74](#control-74): 보안 IRP에 결제 브랜드 및 인수자, 규제 기관, 감독 기관, 이사 및 고객과 같은 주요 이해 관계자에게 적시에 통보할 수 있도록 문서화된 통신 프로세스가 포함되어 있다는 입증 가능한 증거를 제공합니다.
...
<a id="control-75"></a>[제어 75](#control-75): 인시던트 대응 팀의 모든 구성원이 연례 교육 또는 테이블 탑 연습을 완료했다는 입증 가능한 증거를 제공합니다.
...
<a id="control-76"></a>[제어 76](#control-76): 학습된 교훈 또는 조직의 변경 내용에 따라 보안 IRP가 업데이트됨을 보여 주는 입증 가능한 증거를 제공합니다.
...
<a id="control-1b"></a>[제어 1](#control-1b): TLS 구성이 TLS [프로필 구성 요구](../docs/certification-submission-guide.md#appendix-a) 사항 내에서 암호화 요구 사항을 충족하거나 초과한다는 입증 가능한 증거를 제공합니다.
...
<a id="control-2b"></a>[컨트롤 2](#control-2b): 웹 요청을 처리하는 모든 공용 서비스에서 TLS 압축이 비활성화되었다는 입증 가능한 증거를 제공합니다.
...
<a id="control-3b"></a>[제어 3](#control-3b): 모든 사이트에서 TLS HTTP 엄격한 전송 보안을 사용하도록 설정하고 >= 15552000으로 구성되었다는 입증 가능한 증거를 제공합니다.
...
<a id="control-4b"></a>[컨트롤 4](#control-4b): AES, Blowfish, TDES 및 128비트 및 256비트 암호화 키 크기와 같은 암호화 알고리즘을 사용하여 미사용 데이터가 암호화 프로필 요구 사항에 따라 인라인으로 암호화된다는 입증 가능한 증거를 제공합니다.
...
<a id="control-5b"></a>[컨트롤 5](#control-5b): 해시 함수 또는 메시지 인증(HMAC-SHA1)이 암호화 프로필 요구 사항과 함께 미사용 데이터를 인라인으로 보호하는 데만 사용된다는 입증 가능한 증거를 제공합니다.
...
<a id="control-6b"></a>[컨트롤 6](#control-6b): 스토리지 위치 및 데이터를 보호하는 데 사용되는 암호화를 포함하여 저장된 모든 데이터의 인벤토리를 제공합니다.
...
<a id="control-7b"></a>[컨트롤 7](#control-7b): 승인되고 문서화된 데이터 보존 기간이 공식적으로 설정되었다는 입증 가능한 증거를 제공합니다.
...
<a id="control-8b"></a>[컨트롤 8](#control-8b): 보존된 데이터가 정의된 보존 기간과 일치하는 입증 가능한 증거를 제공합니다.
...
<a id="control-9b"></a>[컨트롤 9](#control-9b): 보존 기간 후에 데이터를 안전하게 삭제하기 위한 프로세스가 마련되어 있음을 입증할 수 있는 증거를 제공합니다.
...
<a id="control-10b"></a>[제어 10](#control-10b):P 비즈니스 근거를 포함하여 데이터 또는 암호화 키에 액세스할 수 있는 모든 개인의 목록을 로빙합니다.
...
<a id="control-11b"></a>[컨트롤 11](#control-11b): 데이터 또는 암호화 키에 대한 액세스 권한이 있는 샘플링된 개인이 공식적으로 승인되었다는 입증 가능한 증거를 제공하여 작업 기능에 필요한 권한을 자세히 설명합니다.
...
<a id="control-12b"></a>[제어 12](#control-12b): 데이터 또는 암호화 키에 액세스할 수 있는 샘플링된 개인에게 승인에 포함된 권한만 가지고 있다는 입증 가능한 증거를 제공합니다.
...
<a id="control-13"></a>[컨트롤 13](#control-13b): 고객 데이터가 공유되는 모든 타사 목록을 제공합니다.
...
<a id="control-14b"></a>[제어 14](#control-14b): 고객 데이터를 사용하는 모든 타사에 공유 계약이 적용되었다는 입증 가능한 증거를 제공합니다.
...
<a id="control-15b"></a>[컨트롤 15](#control-15b): 문서화된 SAR(주체 액세스 요청) 프로세스를 제공하고 데이터 주체가 SA를 발생시키는 데 사용할 수 있음을 보여 주는 증거를 제공합니다.
...
<a id="control-16b"></a>[컨트롤 16](#control-16b): SAR에 응답할 때 데이터 주체 데이터의 모든 위치를 식별할 수 있다는 입증 가능한 증거를 제공합니다.
...
<a id="control-17b"></a>[컨트롤 17](#control-17b): 다음과 같이 필요한 모든 요소를 포함해야 하는 개인 정보 보호 알림에 대한 링크를 제공합니다.
