...
ms.locfileid: "61151823"
---
# <a name="microsoft-365-certification---sample-evidence-guide"></a>Microsoft 365 인증 - 샘플 증거 가이드

## <a name="overview"></a>개요

이 가이드는 ISV에 각 인증 컨트롤에 필요한 증거 유형 및 세부 수준에 대한 Microsoft 365 만들어졌습니다. 이 문서에서 공유하는 모든 예제는 컨트롤이 충족되고 있는 것을 입증하는 데 사용할 수 있는 유일한 증거는 나타내지 않지만 필요한 증거 유형에 대한 지침으로만 사용됩니다. 
...
- 예제 증거 지침: 메일링 목록에 구독을 표시하는 방법으로 증거를 제공 할 수 있습니다. 새로 릴리스된 취약점에 대한 보안 원본을 수동으로 검토(예: JIRA 또는 Azure DevOps 사용), 최신 소프트웨어를 찾는 도구(예: 최신 소프트웨어 라이브러리를 찾을 때 Snyk일 수 있는 도구) 또는 인증된 검색을 사용하여 식별하는 Nessus일 수 있습니다.  기한이 지난 소프트웨어).
