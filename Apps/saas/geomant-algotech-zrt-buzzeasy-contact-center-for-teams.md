...
| 앱에서 처리되는 데이터는 무엇인가요? | Microsoft JWT 사용자 및 새로 고침 토큰, Microsoft 애플리케이션 ID, 앱 비밀, M365 Email 콘텐츠 Graph API |
| 앱이 TLS 1.1 이상을 지원하나요? | 예 |
| 앱 또는 기본 인프라가 Microsoft 고객 데이터를 저장하나요? | 예 |
| 데이터베이스에 저장되는 데이터는 무엇인가요? | Microsoft JWT 사용자 및 새로 고침 토큰, Microsoft 애플리케이션 ID, 앱 비밀, M365 Email 콘텐츠 Graph API |
